...
#ifndef VNET_LINALG_STATIC_NUMERIC_VECTOR_H
#define VNET_LINALG_STATIC_NUMERIC_VECTOR_H
...
private:
    ...
    void allocate_fill(T fill) { for (size_t i = 0; i < Size; ++i) arr_[i] = fill; }

    void allocate_from(const VectorStatic &other) { for (size_t i = 0; i < Size; ++i) arr_[i] = other.arr_[i]; }

    void allocate_from(const T (&array)[Size]) { for (size_t i = 0; i < Size; ++i) arr_[i] = array[i]; }
...
using numeric_vector = VectorStatic<double, Size>;

#endif //VNET_LINALG_STATIC_NUMERIC_VECTOR_H
